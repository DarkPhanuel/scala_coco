...
object VehiculeDAO {
 ...
 def addVehicule(vehicule: models.Vehicule): Boolean = {
    val query = "INSERT INTO vehicules (immatriculation,marque, modele, nombrePlaces, annee, statut) VALUES (?, ?, ?, ?, ?, ?)"
    val statement = DB.connection.prepareStatement(query)
    statement.setString(1, vehicule.marque)
    statement.setString(2, vehicule.modele)
    statement.setInt(3, vehicule.nombrePlaces)
    statement.setInt(4, vehicule.annee)
    statement.setString(5, vehicule.immatriculation)
 ...
 def getVehicules : Seq[models.Vehicule] = {
    ...
    while (resultSet.next()) {
      val vehicule = models.Vehicule(
        ...
        nombrePlaces = resultSet.getInt("nombrePlaces"),
