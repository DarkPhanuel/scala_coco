...
object TrajetDAO {
  ...
  // Récupérer les trajets d'un utilisateur (conducteur ou passager)
  def getTrajetsPourUtilisateur(utilisateurId: Int, aVenir: Boolean): List[models.Trajet] = {
    ...
    val sql =
      if (aVenir)
        "SELECT t.* FROM trajets t JOIN utilisateur_trajet ut ON t.id = ut.trajet_id WHERE ut.utilisateur_id = ? AND t.date_depart >= ? ORDER BY t.date_depart ASC"
      else
        "SELECT t.* FROM trajets t JOIN utilisateur_trajet ut ON t.id = ut.trajet_id WHERE ut.utilisateur_id = ? AND t.date_depart < ? ORDER BY t.date_depart DESC"
    try {
  ...
  def rechercherTrajets(villeDepart: String, villeArrivee: String, date: java.time.LocalDate): List[models.Trajet] = {
    val sql = "SELECT * FROM trajets WHERE ville_depart = ? AND ville_arrivee = ? AND date_depart = ?"
    try {
      val stmt = DB.connection.prepareStatement(sql)
